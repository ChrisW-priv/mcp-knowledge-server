...
formatters = {
    'simple': {'format': '[%(levelname)s] %(asctime)s: %(message)s', 'datefmt': '%Y-%m-%dT%H:%M:%S%z'},
    'json': {
        '()': MyJSONFormatter,
        'kwargs': {
            'level': 'levelname',
...
def get_logging_setup(use_sep_thread=False, use_cloud_handler=True):
    ...
    root_handlers = ['queue_handler'] if use_sep_thread else [key for key in handlers.keys()]
...
def log_this_function(func: Callable):
    ...
    def wrapper(*args, **kwargs):
        ...
        logger.debug(f'Function {function_name} called with arguments: {args=}, {kwargs=}')
        try:
            result = func(*args, **kwargs)
        except Exception:
            logger.exception(f'Function {function_name} returned exception after call with {args=}, {kwargs=}!')
            raise
